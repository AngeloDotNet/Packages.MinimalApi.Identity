...
public class ClaimsService(MinimalApiDbContext dbContext, UserManager<ApplicationUser> userManager) : IClaimsService
{
    //public async Task<IResult> GetAllClaimsAsync()
    //{
    //    var query = await dbContext.ClaimTypes.ToListAsync();
    //    var result = query.Select(c => new ClaimResponseModel(c.Id, c.Type, c.Value, c.Default)).ToList();

    //    return result == null ? TypedResults.NotFound(MessageApi.ClaimsNotFound) : TypedResults.Ok(result);
    //}
    ...
    //    if (userHasClaim.Contains(new Claim(model.Type, model.Value)))
    //    {
    //        return TypedResults.BadRequest(MessageApi.ClaimAlreadyAssigned);
    //    }

    //    var claimAssign = new Claim(model.Type, model.Value);
    //    var result = await userManager.AddClaimAsync(user, claimAssign);
    ...
    //    var isClaimAssigned = await CheckClaimIsAssignedToAnyUsersAsync(claim, userList, userManager);
    ...
    //    return Enum.TryParse<ClaimsType>(claimType, true, out _);
    //}

    private static bool CheckClaimTypeIsValid(string claimType)
    {
        return !string.IsNullOrWhiteSpace(claimType) && Enum.TryParse<ClaimsType>(claimType, true, out _);
    }

    //private static async Task<bool> CheckClaimIsAssignedToAnyUsers(ClaimType claimToVerify,
    //    List<ApplicationUser> listUser, UserManager<ApplicationUser> userManager)
    //{
    //    foreach (var user in listUser)
    //    {
    //        var claimsAssigned = await userManager.GetClaimsAsync(user);

    //        if (claimsAssigned.Any(c => c.Type.Equals(claimToVerify.Type, StringComparison.InvariantCultureIgnoreCase)
    //                                    && c.Value.Equals(claimToVerify.Value, StringComparison.InvariantCultureIgnoreCase)))
    //        {
    //            return true;
    //        }
    //    }
    ...
    //private static async Task<bool> CheckClaimIsAssignedToAnyUsersAsync(ClaimType claimToVerify,
    //    List<ApplicationUser> listUser, UserManager<ApplicationUser> userManager)
    //{
    //    var tasks = listUser.Select(async user =>
    //    {
    //        var claimsAssigned = await userManager.GetClaimsAsync(user);

    //        return claimsAssigned.Any(c => c.Type.Equals(claimToVerify.Type, StringComparison.InvariantCultureIgnoreCase)
    //                                    && c.Value.Equals(claimToVerify.Value, StringComparison.InvariantCultureIgnoreCase));
    //    });
